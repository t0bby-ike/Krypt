...
function loaded() {
  form = new formHandler('DataForm', enterActions);
  form._extendedKey = [];
...
function doPbkdf2(decrypting) {
  ...
  if (password.length == 0) {
    if (decrypting) { error("Please enter a password and try again"); }
    return;
  }
...
function doEncrypt() {
  ...
  if (plaintext === '')
  {
    error("Please enter the text you would like to encrypt and try again.");

    return ;
  }

  if (plaintext === '' && v.encryptionResult.length) 
  { 
    return; 
  }
  ...
  {
    error("Please enter a valid password and try again.");
    return;
  }
  ...
  if (v.json) {
    v.encryptionResult = ct;
    v.adata = '';
  } else {
    v.encryptionResult = ct.match(/"ct":"([^"]*)"/)[1]; //"
  }

  let encodedString = btoa(v.encryptionResult);

  v.encryptionResult = encodedString;
  //v.plaintext = '';
  
  form.set(v);
  form.encryptionResult.el.select();
}
...
function doDecrypt() {
  ...
  try 
  {
    let encodedString = atob(ciphertext);
  
    ciphertext= encodedString;
  } 
  catch (e) 
  {
    error("Invalid encrypted text. Please make sure the text you have entered is correct and try again.");
    return;
  }
  
  if (!v.passwordDecrypt && !v.key.length)
  {
  ...
  if (ciphertext.match("{")) {
    /* it's jsonized */
    try {
      v.decryptionResult = sjcl.decrypt(v.passwordDecrypt || v.key, ciphertext, {}, rp);
    } catch(e) {
      error(e);
      return;
    }
    ...
    v.key = rp.key;
    v.ciphertext = "";
    document.getElementById('decryptionResult').select();
  } else {
    /* it's raw */
    ...
    try {
      v.decryptionResult = sjcl.codec.utf8String.fromBits(sjcl.mode[v.mode].decrypt(aes, ciphertext, iv, v.adata, v.tag));     
      document.getElementById('decryptionResult').select();
    } catch (e) {
      error(e);
    }
...
sjcl.mode.ccm = {
    ...
    decrypt: function(a, b, c, d, e) {
        ...
        if (!f.equal(k.tag, a)) throw new sjcl.exception.corrupt("Could not decrypt text. Please make sure the password you have entered is correct and try again.");
        return k.data
    },
